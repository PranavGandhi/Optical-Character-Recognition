...
hidden_layer1={'weight':tf.Variable(tf.random_normal([400,num_hl1])),'biases':tf
.random_normal([num_hl1])}
hidden_layer2={'weight':tf.Variable(tf.random_normal([num_hl1,num_hl2])),'bia
ses':tf.random_normal([num_hl2])}
hidden_layer3={'weight':tf.Variable(tf.random_normal([num_hl2,num_hl3])),'bia
ses':tf.random_normal([num_hl3])}
output_layer={'weight':tf.Variable(tf.random_normal([num_hl3,n_classes])),'bias
es':tf.Variable(tf.random_normal([n_classes]))}
def nn_model(data):
...
cost=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=prediction,l
abels=y))
